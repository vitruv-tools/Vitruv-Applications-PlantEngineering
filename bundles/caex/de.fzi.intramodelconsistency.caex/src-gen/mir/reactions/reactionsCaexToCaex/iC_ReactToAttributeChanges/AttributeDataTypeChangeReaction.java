...
class AttributeDataTypeChangeReaction extends AbstractReactionRealization {
  ...
  public void executeReaction(final EChange change) {
    ...
    mir.reactions.reactionsCaexToCaex.iC_ReactToAttributeChanges.AttributeDataTypeChangeReaction.ActionUserExecution userExecution = new mir.reactions.reactionsCaexToCaex.iC_ReactToAttributeChanges.AttributeDataTypeChangeReaction.ActionUserExecution(this.executionState, this);
  ...
  private boolean matchReplaceChange(final EChange change) {
    if (change instanceof ReplaceSingleValuedEAttribute<?, ?>) {
    	...
    	if (!_localTypedChange.getAffectedFeature().getName().equals("attributeDataType")) {
    		return false;
    	}
  ...
  private static class ActionUserExecution extends AbstractRepairRoutineRealization.UserExecution {
    ...
    public void callRoutine1(final Attribute affectedEObject, final EAttribute affectedFeature, final String oldValue, final String newValue, @Extension final RoutinesFacade _routinesFacade) {
      final EStructuralFeature feature = CAEXFactory.eINSTANCE.createAttribute().eClass().getEStructuralFeature("attributeDataType");
